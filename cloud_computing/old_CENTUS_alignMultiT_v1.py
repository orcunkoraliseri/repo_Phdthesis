...
def add_feature_engineering_columns(data):
    ...
    return data
def preprocess_and_save(df, outCol, save_path="preprocessed_data_multiT.pkl"):
...
def preprocess_data(data, output_columns):
    # FEATURE ENGINEERING
    ...
    data = add_feature_engineering_columns(data)
    ...
    data['Kinship_Diversity'] = data.groupby('Household_ID')['Kinship Relationship'].transform('nunique') / data[
    ...
    nominal_columns = [
        ...
        "Landline Ownership", 'Mobile Phone Ownership',
        ...
        'Kinship_Diversity', "Generational_Diversity", "Gender_Diversity", "Role_Concentration",
        ...
        # Advanced Household Composition
        "Combined_MainIncomeSource_Feature"
    ]

    # Include output columns in nominal columns for encoding
    nominal_columns += output_columns
...
def split_data(data, input_columns, output_columns, test_size=0.2, validation_size=0.1, random_state=42):
    ...
    y = data[output_columns]  # Now handles multiple output columns
...
def ML(df, output_columns,  # List of output columns
       learning_rate=0.0025, num_epochs=30, batch_size=32, ordinal_hidden_size=24, fc_hidden_sizes=(24,),
       dropout_rate=0.0, use_batch_norm=True, save_path="preprocessed_data.pkl"):
    # Load preprocessed data or preprocess if not available
    preprocessed_data, nominal_encodings, nominalCols, ordinalRawCols, aggregated_columns = preprocess_and_save(df, output_columns, save_path)
...
if __name__ == '__main__':
    ...
    output_columns = ['Family Typology', 'School Enrollment', 'Nuclear Family, Occupant Profile',
                      'Nuclear Family, Typology', 'Nuclear Family, Occupant Sequence Number']

    model, train_loader, val_loader, test_loader = ML(data, output_columns)
