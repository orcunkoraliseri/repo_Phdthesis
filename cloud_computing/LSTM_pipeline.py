...
def save_trial_details(study, filename_prefix="TRresults"):
    ...
    folder_name = "LSTMTuningcsvOutputs"
...
def data_preprocess(df):
    ...
    import pickle
    encoder_path = "occupant_activity_label_encoder.pkl"
    # Save the fitted encoder to the provided path
        ...
        for batch_idx, data in enumerate(data_loader):
            try:
                ...
                education_data, employment_data, gender_data, famTypology_data, \
                    ...
                    weekend_data, continuous_data, activity_target, location_target, withNOB_target = [d.to(device) for
                                                                                                       d in data]

                opt.zero_grad()
                # Model's forward pass
                activity_output, location_output, withNOB_output = model(education_data, employment_data, gender_data,
                                                                         famTypology_data,
                                                                         numFamMemb_data, ageClass_data, region_data,
                                                                         MartStatCat_data, 
                                                                         KinsCat_data, OccProfCat_data, FamTypoCat_data,
                                                                         OccSeqNumCat_data,
                                                                         Citizen_data, InterOwn_data,
                                                                         MobPhoneOwn_data, CarOwn_data,
                                                                         FamTypoSimple_data, HomeOwn_data,
                                                                         RoomCount_data, EcoSector_data, JobType_data,
                                                                         OCCinHH_data, season_data,
    ...
    with torch.no_grad():
        for data in data_loader:
            ...
            education_data, employment_data, gender_data, famTypology_data, \
                ...
                FamTypoSimple_data, HomeOwn_data, RoomCount_data, EcoSector_data, JobType_data, \
                OCCinHH_data, season_data, \
                weekend_data, continuous_data, activity_target, location_target, withNOB_target = [d.to(device) for d in
                                                                                                   data]
            ...
            activity_output, location_output, withNOB_output = model(education_data, employment_data, gender_data,
                                                                     ...
                                                                     numFamMemb_data, ageClass_data, region_data,
                                                                     MartStatCat_data, 
                                                                     KinsCat_data, OccProfCat_data,
                                                                     FamTypoCat_data, OccSeqNumCat_data,
                                                                     Citizen_data, InterOwn_data,
                                                                     MobPhoneOwn_data, CarOwn_data,
                                                                     FamTypoSimple_data, HomeOwn_data, RoomCount_data,
                                                                     EcoSector_data, JobType_data,
                                                                     OCCinHH_data, season_data,
                                                                     weekend_data, continuous_data, )
...
def objectiveLSTM_kFold(trial, epochs, df, num_split):
    ...
    num_hidden_layers = trial.suggest_categorical('num_hidden_layers', [2,3])
    hidden_units_options = [300,400,500,600,700, 800, 900, 1000]
    hidden_units = [trial.suggest_categorical(f'hidden_units_l{i}', hidden_units_options) for i in range(num_hidden_layers)]
    embed_size = trial.suggest_categorical('embedding_size', [50])
    batch_size = trial.suggest_categorical('batch_size', [64, 128, 256, 384, 512])
    learning_rate = trial.suggest_categorical('learning_rate', [1e-3, 9e-4, 8e-4, 7e-4, 6e-4, 5e-4])
...
def main_tuningLSTM_kFold(csv_filepath, n_trials, epochs, num_split, df):
    ...
    def objective_wrapper(trial):
        weighted_loss, weighted_accuracy, avg_activity_accuracy, avg_location_accuracy, avg_withNOB_accuracy, last_epoch = objectiveLSTM_kFold(trial, epochs=epochs, df=df, num_split=num_split)
...
def trainEvaluate_afterTuning(df, csv_filepath, rnn_type='LSTM', epochBase=500,):
    ...
    def save_num_features(num_features, filename):
    	with open(filename, 'w') as f:
    		json.dump(num_features, f)

    # Save the dictionary
    save_num_features(num_features, "num_features.json")
    ...
    evalACT_classify_afterTuning(model, X_test, y_activity_test, y_location_test, y_withNOB_test, label_encoder=label_encoder, device=mps_device, save_csv=True, csv_filepath= csv_filepath,arch_type=rnn_type)

def evaluate_and_save_afterTuning(model, X_test, y_activity_test, y_location_test, y_withNOB_test,
                                 label_encoder, device, csv_filepath, arch_type):
    import os
    import pandas as pd
    import torch
    ...
    test_dataset = TensorDataset(
        *[torch.tensor(X_test[:, :, i], dtype=torch.long) for i in range(num_categorical_features)],
        torch.tensor(X_test[:, :, num_categorical_features:], dtype=torch.float),
        torch.tensor(y_activity_test, dtype=torch.long),
        torch.tensor(y_location_test, dtype=torch.float),
        torch.tensor(y_withNOB_test, dtype=torch.float),
    )
    test_loader = DataLoader(test_dataset, batch_size=24, shuffle=False)

    all_p, all_a = {'act':[], 'loc':[], 'nob':[]}, {'act':[], 'loc':[], 'nob':[]}
    model.eval()
    with torch.no_grad():
        for batch in test_loader:
            *cats, cont, ya, yl, yn = [t.to(device) for t in batch]
            pa, pl, pn = model(*cats, cont)
            _, pa = torch.max(pa, 2)
            pl = torch.round(torch.sigmoid(pl)).int()
            pn = torch.round(torch.sigmoid(pn)).int()

            all_p['act'].extend(pa.reshape(-1).cpu().numpy())
            all_p['loc'].extend(pl.reshape(-1).cpu().numpy())
            all_p['nob'].extend(pn.reshape(-1).cpu().numpy())
            all_a['act'].extend(ya.reshape(-1).cpu().numpy())
            all_a['loc'].extend(yl.reshape(-1).cpu().numpy())
            all_a['nob'].extend(yn.reshape(-1).cpu().numpy())

    actual_acts = label_encoder.inverse_transform(all_a['act'])
    pred_acts   = label_encoder.inverse_transform(all_p['act'])

    df = pd.DataFrame({
        # bring in each original column by index
        'Education Degree':    X_flat[:, educationDegree_idx],
        'Employment status':   X_flat[:, employmentStatus_idx],
        'Gender':              X_flat[:, gender_idx],
        'Family Typology':     X_flat[:, famTypology_idx],
        'NumFamMembers':       X_flat[:, numFamMembers_idx],
        'Age Classes':         X_flat[:, ageClass_idx],
        'Region':              X_flat[:, region_idx],
        'Marital Status':      X_flat[:, MartStat_idx],
        'Kinship':             X_flat[:, Kins_idx],
        'Occ Prof':            X_flat[:, OccProf_idx],
        'Fam Typo':            X_flat[:, FamTypo_idx],
        'Occ Seq Num':         X_flat[:, OccSeqNum_idx],
        'Citizenship':         X_flat[:, Citizen_idx],
        'Internet Access':     X_flat[:, InterOwn_idx],
        'Mobile Phone Own':    X_flat[:, MobPhoneOwn_idx],
        'Car Ownership':       X_flat[:, CarOwn_idx],
        'Fam Typo Simple':     X_flat[:, FamTypoSimple_idx],
        'Home Ownership':      X_flat[:, HomeOwn_idx],
        'Room Count':          X_flat[:, RoomCount_idx],
        'Econ Sector/Prof':    X_flat[:, EcoSector_idx],
        'Job Type':            X_flat[:, JobType_idx],
        'OCC in HH':           X_flat[:, OCCinHH_idx],
        'Season':              X_flat[:, season_idx],
        'Weekend':             X_flat[:, weekend_idx],
        # continuous features
        **{f'Cont_{i}': X_flat[:, num_categorical_features + i]
           for i in range(n_features - num_categorical_features)},
        # actual vs. predicted
        'Actual Activity':     actual_acts,
        'Predicted Activity':  pred_acts,
        'Actual Location':     all_a['loc'],
        'Predicted Location':  all_p['loc'],
        'Actual withNOB':      all_a['nob'],
        'Predicted withNOB':   all_p['nob'],
    })
...
if __name__ == '__main__':
    import pandas as pd
    # INPUT PATHS
    data = r'tus_main_EqPadHHID_RAWDATA_31_v4.csv'
    ...
    # LSTM:TUNING CROSS-VALIDATION ---------------------------------------------------------------------------
    #main_tuningLSTM_kFold(csv_filepath=csv_input, epochs=125, n_trials=100, num_split=3, df=df) # defualt epochs= 20-30, default n_trials= 50-100, default n_split=3-5
    trainEvaluate_afterTuning(df=df, csv_filepath=csv_input, rnn_type="LSTM", epochBase=300)
