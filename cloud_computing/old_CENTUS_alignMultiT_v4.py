...
def add_feature_engineering_columns(data):
    feature_engineering_columns = [
        ...
        'Mobile_Phone_to_Total_Devices', "Combined_MainIncomeSource_Feature", 'Stable_Family_Ratio', 'Primary_Kinship_Ratio',
    ...
    data['Connectivity_Score'] = data[['Internet Access', 'Car Ownership',
                                       'Landline Ownership', 'Mobile Phone Ownership']].sum(axis=1)
...
def ML(df, output_columns, base_nominal_columns,
       learning_rate=0.0005, num_epochs=50, batch_size=8, ordinal_hidden_size=24, fc_hidden_sizes=(24,),
    ...
    # Save the fitted preprocessor for later use (e.g., in classification).
    try:
        joblib.dump(preprocessor, 'preprocessor.pkl')
        print("Preprocessor saved as 'preprocessor.pkl' using joblib.")
    except pickle.PicklingError as e:
        print("Joblib failed to pickle the preprocessor. Falling back to dill.")
        import dill
        with open('preprocessor.pkl', 'wb') as f:
            dill.dump(preprocessor, f)
        print("Preprocessor saved as 'preprocessor.pkl' using dill.")
    ...
    output_dropout_rate = {
        "Family Typology": 0.00,  # 0.025, default value
...
def apply_preprocessing(df, nominal_encodings, nominal_columns, ordinal_raw_columns, household_aggregated_columns):
    import pandas as pd
    df_processed = df.copy()
    # Process nominal columns using mappings from training
    for col in nominal_columns:
        if col in df_processed.columns:
            mapping = nominal_encodings.get(col, {})
            df_processed[col] = df_processed[col].map(mapping).fillna(-1).astype(int)
    # Convert ordinal and aggregated columns to numeric
    for col in ordinal_raw_columns + household_aggregated_columns:
        if col in df_processed.columns:
