...
def data_preprocess(df):
    ...
    to_scale = df.columns.difference(one_hot_cols + ['Occupant_Activity',
                                                     "location",
                                                     "withNOBODY",
                                                     "Gender", "Citizenship",  # new additions
                    "Internet Access",
                    "Mobile Phone Ownership",
                    "Car Ownership",])
    ...
    encoder_path = "occupant_activity_encoder.pkl"
...
def data_preprocess_k_fold_split(df, n_splits=3):
    ...
    to_scale = df.columns.difference(one_hot_cols + ['Occupant_Activity',
                                                     "location",
                                                     "withNOBODY",
                                                     "Gender", "Citizenship", "Internet Access",
                                                     "Mobile Phone Ownership", "Car Ownership"])
...
def objectiveLSTM_kFold(trial, epochs, df, num_split):
    ...
    num_hidden_layers = trial.suggest_categorical('num_hidden_layers', [1])
    hidden_units_options = [25, 50,100,150, 200,250, 300,350, 400]
    ...
    batch_size = trial.suggest_categorical('batch_size', [24,48,96,128])
    learning_rate = trial.suggest_categorical('learning_rate', [5e-1, 1e-1, 5e-2, 1e-2, 5e-3, 1e-3])
...
def trainEvaluate_afterTuning(df, csv_filepath, rnn_type='LSTM', epochBase=500,):
    ...
    def save_num_features(num_features, filename):
        ...
        print(f"num_features saved to {filename}")
    ...
    pth_filepath = f"{filename_prefix}_best_model_{rnn_type}.pth"
    #print(pth_filepath)
    if os.path.exists(pth_filepath):
        print(f"Pre-trained model found at {pth_filepath}. Loading model.")
        #model.load_state_dict(torch.load(pth_filepath))
        model.load_state_dict(torch.load(pth_filepath,map_location=device))
    else:
        ...
        history, trained_model = train_and_evaluate_model_tuning(model,
                                                                 ...
                                                                 device=mps_device, w_act=w_act, w_loc=w_loc,
                                                                 w_NOB=w_NOB,
                                                                 checkpoint_path=f"{filename_prefix}_best_model_{rnn_type}.pth",
                                                                 verbose=True, use_tensorboard=True,
                                                                 load_checkpoint=False,
                                                                 record_memory_usage=True,
                                                                 memory_log_path='memory_usage.txt',
                                                                 use_early_stopping=False,
                                                                 use_scheduler=True, )
        torch.cuda.empty_cache()
        torch.save(trained_model.state_dict(), pth_filepath)
        print(f"Training completed and model saved to {pth_filepath}.")
        plot_history(history)

    # tensorboard --logdir=runs/End_to_End_full_embedding # from terminal to see the tensorboard
    model.load_state_dict(torch.load(f"{filename_prefix}_best_model_{rnn_type}.pth"))
    # Classification WITH TEST DATA of the model
    ...
    classify_afterTuning_withNewData(model, X_train, df=df, label_encoder=label_encoder, device=mps_device, csv_filepath= csv_filepath, arch_type=rnn_type)
...
def classify_afterTuning_withNewData(model, X_new, df, label_encoder, device, csv_filepath, arch_type):
    ...
    with torch.no_grad():
        for data in loader:
            features = [d.to(device) for d in data]
            activity_output, location_output, withNOB_output = model(*features)
            _, predicted_activity = torch.max(activity_output, 2)
            predicted_location = torch.round(torch.sigmoid(location_output)).int()
            #predicted_withNOB = torch.round(torch.sigmoid(withNOB_output)).int()
            predicted_withNOB = 1 - torch.round(torch.sigmoid(withNOB_output)).int()
...
def evalACT_classify_afterTuning(model, X_test, y_activity_test, y_location_test, y_withNOB_test, label_encoder, device, csv_filepath, arch_type, save_csv=False, ):
    ...
    if save_csv:
        ...
        conf_matrix_filename = filename_prefix + f'_confusion_matrix_{arch_type}.csv'
...
if __name__ == '__main__':
    import pandas as pd
    # INPUT PATHS
    #data = r'dataset_TUS_equalized/tus_main_EqPad100HHID_RAWDATA_31_100.csv'  # 60 minute intervals
    data = r'dataset_TUS_equalized/tus_main_EqPad500HHID_RAWDATA_31.csv'  # 60 minute intervals
    ...
    # LSTM:TUNING CROSS-VALIDATION ---------------------------------------------------------------------------
    #main_tuningLSTM_kFold(csv_filepath=csv_input, epochs=2, n_trials=5, num_split=3, df=df) # defualt epochs= 20-30, default n_trials= 50-100, default n_split=3-5
    #trainEvaluate_afterTuning(df=df, csv_filepath=csv_input, rnn_type="LSTM", epochBase=100)
