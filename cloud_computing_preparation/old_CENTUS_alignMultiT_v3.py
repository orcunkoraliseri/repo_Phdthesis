...
def add_feature_engineering_columns(data):
    feature_engineering_columns = [
        ...
        'Mobile_Phone_to_Total_Devices', "Combined_MainIncomeSource_Feature"
    ]
    # Normalized Ratios
...
def preprocess_and_save(df, outCol,base_nominal_columns, save_path="preprocessed_data_multiT.pkl"):
    if os.path.exists(save_path):
        ...
        preprocessed_data = preprocess_data(df, outCol, base_nominal_columns=base_nominal_columns)
...
def preprocess_data(data, output_columns, base_nominal_columns):
    ...
    # Flattened list of nominal columns
...
def ML(df, output_columns, base_nominal_columns,  # List of output columns
       learning_rate=0.0025, num_epochs=30, batch_size=32, ordinal_hidden_size=24, fc_hidden_sizes=(24,),
       dropout_rate=0.0, use_batch_norm=True, save_path="preprocessed_data_v3.pkl"):
    # Load preprocessed data or preprocess if not available
    preprocessed_data, nominal_encodings, nominalCols, ordinalRawCols, aggregated_columns = preprocess_and_save(df, output_columns, base_nominal_columns, save_path)

    # Remove output columns from nominal columns to prevent leakage
    nominalCols = [col for col in nominalCols if col not in output_columns]

    # Ensure household aggregated columns are included in input_columns
    input_columns = nominalCols + ordinalRawCols + aggregated_columns
    print("Input Columns:", input_columns)

    # Split the data
    splits = split_data(preprocessed_data, input_columns, output_columns, test_size=0.2, validation_size=0.2)
    ...
    embedding_sizes = calculate_embedding_sizes({col: nominal_encodings[col] for col in nominalCols})

    # Define nominal and ordinal columns
    ordinalCols = ordinalRawCols + aggregated_columns

    # Create datasets
    train_dataset = CustomDataset(X_train, y_train, nominalCols, ordinalCols, output_columns)
    val_dataset = CustomDataset(X_val, y_val, nominalCols, ordinalCols, output_columns)
    test_dataset = CustomDataset(X_test, y_test, nominalCols, ordinalCols, output_columns)
    ...
    # Output sizes for each target column
    output_sizes = {col: len(y_train[col].unique()) for col in output_columns}

    output_dropout_rate = {
        "Family Typology": 0.05,  # Add dropout to this head
        "Nuclear Family, Occupant Profile": 0.15,
        'Nuclear Family, Typology': 0.15,
        'Nuclear Family, Occupant Sequence Number': 0.21,
    }

    # Initialize the multi-output model with separate dropout rates
    model = MultiOutputNeuralNetwork(
        embedding_sizes=embedding_sizes,
        num_ordinal_features=len(ordinalCols),
        output_sizes=output_sizes,  # Dictionary of output column sizes
        ordinal_hidden_size=ordinal_hidden_size,
        fc_hidden_sizes=fc_hidden_sizes,
        dropout_rate=dropout_rate,
        use_batch_norm=use_batch_norm,
        output_dropout_rate=output_dropout_rate,  # Pass the output-specific dropout rates
    )

    print(model)

    # Define optimizer and criterion for each output column
    ...
    # Call the function to compute feature importance (can be modified for multi-output)
    #compute_multi_output_feature_importance(model, test_dataset, nominalCols, ordinalCols + aggregated_columns)
    return model, train_loader, val_loader, test_loader

#CLASSIFICATION---------------------------------------------------------------------------------------------------------
def classify_new_data(model_path, new_data, output_columns, base_nominal_columns):
    """
    Classifies new data using the trained model.

    Parameters:
        model_path (str): Path to the saved .pth model file.
        new_data (DataFrame): New data for classification.
        output_columns (list): List of output columns for classification.
        base_nominal_columns (list): List of base nominal columns.

    Returns:
        dict: Dictionary of predictions for each output column.
    """
    # Perform feature engineering and preprocessing
    new_data_preprocessed, nominal_encodings, nominal_columns, ordinal_raw_columns, household_aggregated_columns = preprocess_data(
        new_data, output_columns, base_nominal_columns
    )

    # Create input columns based on the preprocessing
    input_columns = nominal_columns + ordinal_raw_columns + household_aggregated_columns

    # Convert data to tensors
    x_nominal = torch.tensor(new_data_preprocessed[nominal_columns].values, dtype=torch.long)
    x_ordinal = torch.tensor(new_data_preprocessed[ordinal_raw_columns + household_aggregated_columns].values, dtype=torch.float32)

    # Load the trained model
    model = MultiOutputNeuralNetwork(
        embedding_sizes=calculate_embedding_sizes({col: nominal_encodings[col] for col in nominal_columns}),
        num_ordinal_features=len(ordinal_raw_columns + household_aggregated_columns),
        output_sizes={col: len(nominal_encodings[col]) for col in output_columns},
        ordinal_hidden_size=24,
        fc_hidden_sizes=(24,),
        dropout_rate=0.0,
        use_batch_norm=True,
        output_dropout_rate={
            "Family Typology": 0.05,
            "Nuclear Family, Occupant Profile": 0.15,
            'Nuclear Family, Typology': 0.15,
            'Nuclear Family, Occupant Sequence Number': 0.21,
        }
    ...
    for col in output_columns:
        ...
        reverse_mapping = {v: k for k, v in nominal_encodings[col].items()}
...
if __name__ == '__main__':
    ...
    output_columns = ['Family Typology',  'Nuclear Family, Occupant Profile',
                      'Nuclear Family, Typology', 'Nuclear Family, Occupant Sequence Number']
    ...
    base_nominal_columns = [ 'Region', 'Employment status', 'Gender', 'Marital Status', 'Kinship Relationship',
                             ...
                             "Family_Typology_Simple", 'Internet Access', 'Car Ownership', "Landline Ownership",
